function rawData = make_glm_trials_from_Msorted(Msorted,varargin)
    ...
    duration = round(diff(params.spikeWindowS.(params.ref_event))*params.samplingFreq);
    ...
    for t = 1:Msorted.nTrials
        trial_start_time = Msorted.Trials.stateTimes.(params.ref_event)(t) + params.spikeWindowS.(params.ref_event)(1);
        ...
        for c = 1:length(Msorted.tt)
            rawData.trial(t).(['sptrain',num2str(c)]) = round( (Msorted.spike_time_s.(params.ref_event){c}{t} - params.spikeWindowS.(params.ref_event)(1) ) * params.samplingFreq);
        end
    end
