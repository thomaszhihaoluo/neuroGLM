function [] = examine_pre_sound(Stats, varargin)
p=inputParser;
...
n_cond = numel(Stats);
for i = 1:n_cond
    if any(kernel_names == "sound_on_left") && ...
       any(kernel_names == "sound_on_right")
            R_minus_L = Kernels.sound_on_right.ws{i} - Kernels.sound_on_left.ws{i};
            R_minus_L = R_minus_L(:, Kernels.sound_on_left.tr{i}<=0);
...
for j = i+1:n_cond
    ...
    title('peak R-L choice kernel before stim')
